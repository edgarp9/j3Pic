...
| 설정 저장/로드 | 구현. Windows는 `APPDATA`/`LOCALAPPDATA`, Linux는 `XDG_CONFIG_HOME` 또는 `$HOME/.config` 아래 `j3Pic/config.txt`를 사용한다. 표준 config 디렉터리가 없으면 로드는 기본값으로 계속하고 종료 저장은 건너뛴다. Linux config replace는 temp file sync 후 rename하고 parent directory를 fsync해 Windows `MOVEFILE_WRITE_THROUGH` 기준에 맞춘다. |
...
GDK_BACKEND=x11 GSETTINGS_BACKEND=memory XDG_CONFIG_HOME=<temp> cargo run --quiet --bin j3pic -- <temp>/smoke.png
OK. ImageMagick `convert`로 만든 64x48 PNG 기준으로 GTK 주 창 `960x640`, title `smoke.png - j3Pic`, F11 후 `_NET_WM_STATE_FULLSCREEN`, Escape 후 fullscreen state 제거, `q` 종료, config 저장을 확인했다. stderr에는 현재 세션의 AT-SPI bus 경고만 출력됐다.

GDK_BACKEND=x11 GSETTINGS_BACKEND=memory XDG_CONFIG_HOME=<temp> cargo run --quiet --bin j3pic -- <temp>/smoke.png + xdotool
OK. `Menu`, `Escape`, `Shift+F10`, `Escape` 후 포커스가 viewer로 돌아오고, `R`/`Shift+R` 제목 변경, F11/Escape fullscreen 왕복, `q` 종료와 config 저장을 확인했다. stderr에는 현재 세션의 AT-SPI bus 경고만 출력됐다.

GDK_BACKEND=x11 GSETTINGS_BACKEND=memory XDG_CONFIG_HOME=<temp1> cargo run --quiet --bin j3pic -- <temp>/first.png
GDK_BACKEND=x11 GSETTINGS_BACKEND=memory XDG_CONFIG_HOME=<temp2> cargo run --quiet --bin j3pic -- <temp>/second.png
OK. `NON_UNIQUE` 실행 정책으로 `first.png - j3Pic`, `second.png - j3Pic` 두 창이 동시에 뜨고, 각 창을 종료한 뒤 별도 config 저장을 확인했다. stderr에는 현재 세션의 AT-SPI bus 경고만 출력됐다.
