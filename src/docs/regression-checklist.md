...
대신 사용자 설정을 건드리지 않도록 임시 디렉터리에 `j3pic.exe`를 복사해
실제 실행했다. 빈 시작은 title `j3Pic`, 시작 이미지 `icon.ico` 인자는 title
`icon.ico - j3Pic`, `CloseMainWindow` 종료 코드 `0`, 복사한 실행파일 옆
`j3pic.toml` 저장을 확인했다.
...
`target/debug/j3pic.exe`를 임시 디렉터리에 복사해 실행하고, 복사한 실행파일 옆
`j3pic.toml`로 사용자 설정을 격리했다.
...
- `WM_CLOSE` 종료와 실행파일 옆 `j3pic.toml` 저장.
- 설정창 값 변경 후 `확인`, 앱 종료, 재실행 후 값 유지 확인.
- 설정창 `취소`와 `X` 닫기는 `j3pic.toml`을 변경하지 않음.
- 설정창 `기본값` 후 `확인`하면 기본값 기준 `j3pic.toml` 저장.
- `j3pic.toml` 삭제 후 기본값으로 실행.
- 손상된 `j3pic.toml`(`version=2`와 malformed line)가 앱 시작을 막지 않고 기본값으로 설정창 표시.
...
- 여러 설정값을 변경하고 `확인`하면 `j3pic.toml`에 즉시 저장된다.
- 앱 종료 후 같은 임시 실행파일 디렉터리로 재실행하면 설정창 컨트롤이 저장값으로 초기화된다.
- `취소`와 `X` 닫기는 draft를 버리고 저장 파일을 변경하지 않는다.
- `기본값` 후 `확인`하면 사용자 설정 key가 `AppConfig::default()` 기준으로 저장된다.
- `j3pic.toml`이 없으면 기본값으로 실행되고 설정창이 정상 표시된다.
- 손상된 `j3pic.toml`은 앱 시작을 막지 않고 기본값으로 복구된다.
...
- 앱을 임시 디렉터리에 복사해 실행하거나 기존 실행파일 옆 `j3pic.toml`을 백업해 사용자 설정을 격리한다.
- 앱 실행 후 이미지가 없는 상태와 이미지가 로드된 상태에서 각각 우클릭 컨텍스트 메뉴를 연다.
- 컨텍스트 메뉴 마지막 항목이 `설정`이고, 선택하면 `j3Pic 설정` modal dialog가 열린다.

수동 확인 항목:

- `취소`와 창 닫기: 값을 바꾼 뒤 `취소` 또는 `X`로 닫으면 현재 viewer 동작과 `j3pic.toml`이 바뀌지 않는다.
...
- 구버전/누락 config: `j3pic.toml`이 없거나 새 사용자 설정 key가 없는 `version=1` 파일만 있어도 앱이 기본값으로 실행되고 설정창이 정상 표시된다. `version=2`나 손상된 라인은 기본값으로 복구되어 앱 시작을 막지 않는다.
...
actual app smoke with temp executable directory
