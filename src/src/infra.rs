...
mod tests {
    ...
    use super::config_store::app_config_file_path_for_executable;
    use super::{
    ...
    #[test]
    fn app_config_file_path_uses_executable_directory_and_stem() {
        #[cfg(target_os = "windows")]
        let executable_path = Path::new(r"C:\Apps\j3pic.exe");
        #[cfg(target_os = "windows")]
        let expected_path = PathBuf::from(r"C:\Apps\j3pic.toml");
        #[cfg(not(target_os = "windows"))]
        let executable_path = Path::new("/opt/j3pic");
        #[cfg(not(target_os = "windows"))]
        let expected_path = PathBuf::from("/opt/j3pic.toml");

        assert_eq!(
            app_config_file_path_for_executable(executable_path),
            Some(expected_path)
        );
    }

    #[test]
    fn app_config_file_path_requires_executable_directory() {
        assert_eq!(
            app_config_file_path_for_executable(Path::new("j3pic")),
            None
        );
    }
...
",
    ...
    fn config_replace_renames_file_and_syncs_parent_directory() {
        ...
        fs::create_dir_all(&dir).expect("test dir");
        let temporary_path = dir.join("j3pic.toml.tmp");
        let path = dir.join("j3pic.toml");
    ...
    fn save_config_writes_temp_then_loads_roundtrip() {
        ...
        assert!(!path.with_file_name("j3pic.toml.tmp").exists());
        let _ = fs::remove_dir_all(dir);
    }
    ...
    fn save_config_does_not_reuse_existing_fixed_temporary_file() {
        ...
        fs::create_dir_all(&dir).expect("test dir");
        let path = dir.join("j3pic.toml");
        let fixed_temporary_path = path.with_file_name("j3pic.toml.tmp");
        ...
        assert_config_temporary_files_removed(&dir, "j3pic.toml");
        let _ = fs::remove_dir_all(dir);
    }
